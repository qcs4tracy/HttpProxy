...
void CommunicatingSocket::send(const void *buffer, size_t bufferLen)
throw(SocketException) {
    ssize_t nsent_;
    while (bufferLen > 0) {
        nsent_ = ::send(sockDesc, (raw_type *) buffer, bufferLen, 0);
        if (nsent_ < 0) {
...
ssize_t CommunicatingSocket::recv(void *buffer, int bufferLen)
throw(SocketException) {
    ssize_t rtn;
